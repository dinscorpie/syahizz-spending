...
FOR SELECT USING (
  ...
  EXISTS (
    SELECT 1 FROM public.family_members fm
    WHERE fm.family_id = receipts.family_id AND fm.user_id = auth.uid()
  )
);
...
FOR UPDATE USING (
  ...
  EXISTS (
    SELECT 1 FROM public.family_members fm
    WHERE fm.family_id = receipts.family_id AND fm.user_id = auth.uid() AND fm.role = 'admin'
  )
);
...
SET added_by = user_id,
    family_id = (
      ...
      WHERE fm.user_id = receipts.user_id 
      LIMIT 1
    )
